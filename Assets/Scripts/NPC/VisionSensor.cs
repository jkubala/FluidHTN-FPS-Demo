...
public class VisionSensor : MonoBehaviour
{
	...
	[SerializeField] float visionVertFocusAngleUp = 25f;
	...
	[SerializeField] float visionFocusAngle = 60f;
	...
	[SerializeField] float multFocusFar = 0.1f;
	...
	void Start()
	{
		...
		detectionDirectionUpdater = GameObject.FindGameObjectWithTag("DetectionCollisionUpdater").GetComponent<DetectionDirectionUpdater>();
		CheckRangeValues();
	}

	void CheckRangeValues()
	{
		if (visionFocusFar < visionFocusDistance) { Debug.LogError("visionFocusFar is less than visionFocusClose on object " + gameObject.name); }
		if (visionFocusFar < visionFocusDistance) { Debug.LogError("visionFocusFar is less than visionFocusNearPeripheralBorder on object " + gameObject.name); }
		if (visionFocusDistance < visionPeripheralNearDistance) { Debug.LogError("visionFocusNearPeripheralBorder is less than visionPeripheralNearMidBorder on object " + gameObject.name); }
		if (multFocusClose < multFocusFar) { Debug.LogError("multFocusClose is less than multFocusFar on object " + gameObject.name); }
		if (multPeripheralClose < multPeripheralFar) { Debug.LogError("multPeripheralClose is less than multPeripheralFar on object " + gameObject.name); }
	}
	...
	void Update()
	{
		...
		for (int i = 0; i < aiAgent._context.enemiesSpecificData.Count; i++)
		{
			...
			AIContext.TargetData currentTargetData = aiAgent._context.enemiesSpecificData[currentTarget];
			...
			else
			{
				...
				if (currentTarget.IsPlayer)
				{
					...
					detectionDirectionUpdater.UpdateGUIFill(gameObject, currentTargetData.awarenessOfThisTarget / aiAgent._context.alertAwarenessThreshold);
				}
			}
		}
	}
	...
	float GetVisionModifier(HumanTarget target, AIContext.TargetData targetData)
	{
	...
	float GetRaycastModifier(HumanTarget targetToRayCast, AIContext.TargetData targetData)
	{
		...
		foreach (VisibleBodyPart bodyPart in targetToRayCast.bodyPartsToRaycast)
		{
			...
			if (bodyPartVisionModifier > 0)
			{
				...
				overallRayCastModifier = Mathf.Min(overallRayCastModifier + (bodyPart.visibilityModifier * bodyPartVisionModifier), 100);
			}
		}
	...
	float CalculateRayCastModifierForBodyPart(VisibleBodyPart bodyPart)
	{
		...
		if (Physics.Raycast(origin, direction, out RaycastHit hit, visionFocusFar, raycastMask)
			&& hit.transform.TryGetComponent(out VisibleBodyPart bodyPartHit) && bodyPartHit.owner == bodyPart.owner)
		{
			return 1f;
		}
	...
	float GetHorizontalVisionConeModifier(float distanceToTarget, float horizontalAngle, HumanTarget target)
	{
		...
		if (horizontalAngle < visionFocusAngle)
		{
			...
			else if (distanceToTarget < visionFocusFar)
			{
				...
				{
					horizontalModifier = Mathf.Lerp(multFocusClose, multFocusFar, (distanceToTarget - visionFocusDistance) / farVisionRatio);
				}
			}
		}
		...
		else if (horizontalAngle < visionPeripheralNearAngle)
		{
			...
			{
				horizontalModifier = Mathf.Lerp(multPeripheralClose, multPeripheralFar, (distanceToTarget - visionPeripheralNearDistance) / peripheralNearVisionRatio);
			}
		}
		...
		else if (horizontalAngle < visionPeripheralMidAngle)
		{
			...
			else if (distanceToTarget < visionPeripheralNearDistance)
			{
				if (!target.IsCrouching)
				{
					horizontalModifier = Mathf.Lerp(multPeripheralClose, multPeripheralFar, (distanceToTarget - visionPeripheralRearDistance) / peripheralMidVisionRatio);
				}
			}
		}
	...
	float GetVerticalVisionConeModifier(float distanceToTarget, float verticalAngle, HumanTarget target)
	{
		...
		if (verticalAngle > 0 && verticalAngle > visionVertFocusAngleUp)
		{
			...
			{
				verticalModifier = Mathf.Lerp(multVertUpper, multFocusVertical, (visionVertUpperAngle - verticalAngle) / verticalTopRatio);
			}
		}
		...
		else if (Mathf.Abs(verticalAngle) > visionVertFocusAngleDown)
		{
			...
			{
				verticalModifier = Mathf.Lerp(multVertLower, multFocusVertical, (visionVertLowerAngle - verticalAngle) / verticalBotRatio);
			}
		}
	...
	public class ExampleEditor : Editor
	{
		public void OnSceneGUI()
		{
			...
			if (t.enabled)
			{
				...
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionFocusAngle, 0) * t.thisTarget.eyes.forward, t.visionFocusAngle * 2, t.visionFocusFar);

				// Focus near
				Handles.color = new Color(1f, 0f, 0f, 0.1f);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionFocusAngle, 0) * t.thisTarget.eyes.forward, t.visionFocusAngle * 2, t.visionFocusDistance);

				// Near peripheral far
				Handles.color = new Color(0.5f, 0.5f, 0.5f, 0.1f);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionPeripheralNearAngle, 0) * t.thisTarget.eyes.forward, t.visionPeripheralNearAngle - t.visionFocusAngle, t.visionFocusDistance);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, t.visionPeripheralNearAngle, 0) * t.thisTarget.eyes.forward, -t.visionPeripheralNearAngle + t.visionFocusAngle, t.visionFocusDistance);

				// Near peripheral close
				Handles.color = new Color(1f, 0.5f, 0.05f, 0.1f);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionPeripheralNearAngle, 0) * t.thisTarget.eyes.forward, t.visionPeripheralNearAngle - t.visionFocusAngle, t.visionPeripheralNearDistance);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, t.visionPeripheralNearAngle, 0) * t.thisTarget.eyes.forward, -t.visionPeripheralNearAngle + t.visionFocusAngle, t.visionPeripheralNearDistance);

				// Mid peripheral far
				Handles.color = new Color(0.5f, 0.5f, 0.5f, 0.1f);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionPeripheralMidAngle, 0) * t.thisTarget.eyes.forward, t.visionPeripheralMidAngle - t.visionPeripheralNearAngle, t.visionPeripheralNearDistance);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, t.visionPeripheralMidAngle, 0) * t.thisTarget.eyes.forward, -t.visionPeripheralMidAngle + t.visionPeripheralNearAngle, t.visionPeripheralNearDistance);

				// Mid peripheral close
				Handles.color = new Color(1f, 0.5f, 0.05f, 0.1f);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionPeripheralMidAngle, 0) * t.thisTarget.eyes.forward, t.visionPeripheralMidAngle - t.visionPeripheralNearAngle, t.visionPeripheralRearDistance);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, t.visionPeripheralMidAngle, 0) * t.thisTarget.eyes.forward, -t.visionPeripheralMidAngle + t.visionPeripheralNearAngle, t.visionPeripheralRearDistance);

				// Peripheral rear
				Handles.color = new Color(1f, 0.5f, 0.05f, 0.1f);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, -t.visionPeripheralRearAngle - t.visionPeripheralMidAngle, 0) * t.thisTarget.eyes.forward, t.visionPeripheralRearAngle, t.visionPeripheralRearDistance);
				Handles.DrawSolidArc(t.thisTarget.eyes.position, Vector3.up, Quaternion.Euler(0, t.visionPeripheralRearAngle + t.visionPeripheralMidAngle, 0) * t.thisTarget.eyes.forward, -t.visionPeripheralRearAngle, t.visionPeripheralRearDistance);
